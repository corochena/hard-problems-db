...
tags:
  ...
  - bifurcation
difficulty: Hard
key_insight: "The steady-state angle depends on the dimensionless parameter omega^2 * R / g, exhibiting a pitchfork bifurcation."
---
...
2. Determine the equilibrium positions $\theta_0$ and analyze their stability.
...
- Pay attention to the effective potential $V_{\text{eff}}(\theta)$ when examining stability.
...
def effective_potential(theta, omega, R, g=9.81):
    return -m * g * R * np.cos(theta) - 0.5 * m * (R * omega)**2 * np.sin(theta)**2

# Parameters
R = 0.5  # meters
g = 9.81
omega_crit = np.sqrt(g / R)

print(f"Critical Angular Velocity: {omega_crit:.2f} rad/s")